  ...
  module ModelTemplate
    ...
    class Ubuntu < ProjectRazor::ModelTemplate::Base
      ...
      def mk_call(node, policy_uuid)
        ...
        case @current_state
          # We need to reboot
          when :init, :preinstall, :postinstall, :os_validate, :os_complete, :broker_check, :broker_fail, :broker_success, :broker_wait
            ret = [:reboot, {}]
      ...
      def boot_call(node, policy_uuid)
        ...
        case @current_state
          ...
          when :postinstall, :os_complete, :broker_check, :broker_fail, :broker_success, :broker_wait, :complete_no_broker
            ret = local_boot(node)
          when :timeout_error, :error_catch
